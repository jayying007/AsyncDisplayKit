...
- (void)_recursivelyRasterizeSelfAndSublayersWithIsCancelledBlock:(asdisplaynode_iscancelled_block_t)isCancelledBlock
    ...
    CGPoint origin = CGPointMake(position.x - scaledBoundsSize.width * anchorPoint.x, position.y - scaledBoundsSize.height * anchorPoint.y);
    CGRect frame = CGRectMake(origin.x, origin.y, bounds.size.width, bounds.size.height);

    // Get the display block for this node.
    asyncdisplaykit_async_transaction_operation_block_t displayBlock = [self _displayBlockWithAsynchronous:NO
...
- (asyncdisplaykit_async_transaction_operation_block_t)_displayBlockWithAsynchronous:(BOOL)asynchronous
    ...
    ASDisplayNodeAssert(rasterizing || ![self __rasterizedContainerNode],
