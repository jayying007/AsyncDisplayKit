...
+ (void)initialize {
    ...
    ASDisplayNodeAssert(!ASDisplayNodeSubclassOverridesSelector(self, @selector(measure:)),
                        @"Subclass %@ must not override measure method",
                        NSStringFromClass(self));
    ASDisplayNodeAssert(!ASDisplayNodeSubclassOverridesSelector(self, @selector(recursivelyReclaimMemory)),
                        @"Subclass %@ must not override recursivelyReclaimMemory method",
                        NSStringFromClass(self));
}
...
- (void)_initializeInstance {
    ...
    _flags.implementsDisplay = [[self class] respondsToSelector:@selector(drawRect:withParameters:isCancelled:isRasterizing:)] ||
                               [self.class respondsToSelector:@selector(displayWithParameters:isCancelled:)];
...
static bool disableNotificationsForMovingBetweenParents(ASDisplayNode *from, ASDisplayNode *to) {
    if (!from || !to)
        return NO;
    if (from->_flags.isSynchronous)
        return NO;
    if (to->_flags.isSynchronous)
        return NO;
    if (from->_flags.inWindow != to->_flags.inWindow)
        return NO;
    return YES;
}
...
- (void)willEnterHierarchy {
    ASDisplayNodeAssertMainThread();
    ASDisplayNodeAssert(_flags.isInEnterHierarchy,
...
- (void)didExitHierarchy {
    ASDisplayNodeAssertMainThread();
    ASDisplayNodeAssert(_flags.isInExitHierarchy,
...
- (void)setAsyncdisplaykit_node:(ASDisplayNode *)node {
    objc_setAssociatedObject(self,
                             ...
                             node,
                             OBJC_ASSOCIATION_ASSIGN); // Weak reference to avoid cycle, since the node retains the view.
}
