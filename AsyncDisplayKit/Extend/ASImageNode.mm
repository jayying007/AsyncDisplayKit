...
- (id)initWithCrop:(BOOL)cropEnabled
                opaque:(BOOL)opaque
                 image:(UIImage *)image
                bounds:(CGRect)bounds
         contentsScale:(CGFloat)contentsScale
       backgroundColor:(UIColor *)backgroundColor
                  tint:(ASImageNodeTint)tint
           contentMode:(UIViewContentMode)contentMode
              cropRect:(CGRect)cropRect
imageModificationBlock:(asimagenode_modification_block_t)imageModificationBlock {
    self = [self init];
    if (!self)
        return nil;
...
- (NSString *)description {
    ...
    [NSString stringWithFormat:
              @"<%@ : %p image:%@ cropEnabled:%@ opaque:%@ bounds:%@ contentsScale:%.2f backgroundColor:%@ tint:%zd contentMode:%@ cropRect:%@>",
              [self class],
              self,
              self.image,
              @(self.cropEnabled),
              ...
              self.tint,
              ASDisplayNodeNSStringFromUIContentMode(self.contentMode),
              NSStringFromCGRect(self.cropRect)];
}
...
- (NSObject *)drawParametersForAsyncLayer:(_ASDisplayLayer *)layer;
{
    ...
    return
    [[_ASImageNodeDrawParameters alloc] initWithCrop:_cropEnabled
                                              opaque:self.opaque
                                               image:self.image
                                              bounds:(hasValidCropBounds ? _cropDisplayBounds : self.bounds)contentsScale:self.contentsScaleForDisplay
...
+ (UIImage *)displayWithParameters:(_ASImageNodeDrawParameters *)parameters isCancelled:(asdisplaynode_iscancelled_block_t)isCancelled {
    ...
    BOOL imageHasAlpha = alphaInfo == kCGImageAlphaFirst || alphaInfo == kCGImageAlphaLast || alphaInfo == kCGImageAlphaPremultipliedFirst
                         || alphaInfo == kCGImageAlphaPremultipliedLast;

    BOOL contentModeSupported = contentMode == UIViewContentModeScaleAspectFill || contentMode == UIViewContentModeScaleAspectFit;

    CGSize backingSize;
    CGRect imageDrawRect;

    if (boundsSizeInPixels.width * contentsScale < 1.0f || boundsSizeInPixels.height * contentsScale < 1.0f || imageSizeInPixels.width < 1.0f
    ...
    if (backingSize.width <= 0.0f || backingSize.height <= 0.0f || imageDrawRect.size.width <= 0.0f || imageDrawRect.size.height <= 0.0f) {
        return nil;
    }
