...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"<%@: %p; text = \"%@\"; truncation string = \"%@\"; frame = %@>",
                                      self.class,
                                      self,
                                      plainString,
                                      truncationString,
                                      self.nodeLoaded ? NSStringFromCGRect(self.layer.frame) : nil];
}
...
- (CGSize)calculateSizeThatFits:(CGSize)constrainedSize {
    ...
    ASDisplayNodeAssert(constrainedSizeForText.width >= 0,
                        @"Constrained width for text (%f) after subtracting shadow padding (%@) is too narrow",
                        constrainedSizeForText.width,
                        NSStringFromUIEdgeInsets(shadowPadding));
    ASDisplayNodeAssert(constrainedSizeForText.height >= 0,
                        @"Constrained height for text (%f) after subtracting shadow padding (%@) is too short",
                        constrainedSizeForText.height,
                        NSStringFromUIEdgeInsets(shadowPadding));
    ...
    ASDisplayNodeAssert(renderSizePlusShadowPadding.width >= 0,
                        @"Calculated width for text with shadow padding (%f) is too  narrow",
                        constrainedSizeForText.width);
    ASDisplayNodeAssert(renderSizePlusShadowPadding.height >= 0,
                        @"Calculated height for text with shadow padding (%f) is too short",
                        constrainedSizeForText.height);
...
+ (void)drawRect:(CGRect)bounds
  withParameters:(ASTextNodeDrawParameters *)parameters
...
- (id)linkAttributeValueAtPoint:(CGPoint)point attributeName:(out NSString **)attributeNameOut range:(out NSRange *)rangeOut {
    return [self _linkAttributeValueAtPoint:point attributeName:attributeNameOut range:rangeOut inAdditionalTruncationMessage:NULL];
}
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (([self _pendingLinkTap] || [self _pendingTruncationTap]) && [gestureRecognizer isKindOfClass:[UITapGestureRecognizer class]]
    ...
    if (_attributedString.length > 0) {
        ...
        [truncationString enumerateAttributesInRange:NSMakeRange(0, truncationString.length)
                                          ...
                                          usingBlock:^(NSDictionary *attributes, NSRange range, BOOL *stop) {
                                              ...
                                              [NSMutableDictionary dictionaryWithDictionary:originalStringAttributes];
