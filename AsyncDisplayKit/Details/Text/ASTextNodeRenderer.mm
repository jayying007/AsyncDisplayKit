...
- (void)_invalidateLayout {
    ...
    if (_visibleRange.length < originalStringLength && _truncationString.length > 0) {
        ...
        [_textStorage replaceCharactersInRange:truncationReplacementRange withAttributedString:_truncationString];
                                 ...
                                 if (CGRectIsEmpty(usedRect)) {
                                     ...
                                     for (NSUInteger i = lineGlyphRange.location; i < NSMaxRange(lineGlyphRange) && i < string.length; i++) {
                                         ...
                                         if (!CGRectIsEmpty(properGlyphRect)) {
                                             lineRect = CGRectIsNull(lineRect) ? properGlyphRect : CGRectUnion(lineRect, properGlyphRect);
                                         }
                                     }
                                 }
...
- (CGRect)_rectForGlyphAtIndex:(NSUInteger)glyphIndex measureOption:(ASTextNodeRendererMeasureOption)measureOption {
    ASDN::MutexLocker l(_textKitLock);
    ...
    CTFontRef font = (__bridge_retained CTFontRef)[_textStorage attribute:NSFontAttributeName atIndex:charIndex effectiveRange:NULL];
    if (font == nil) {
    ...
    CGFloat advance =
    (glyph == kCGFontIndexInvalid) ? glyphRect.size.width : CTFontGetAdvancesForGlyphs(font, kCTFontOrientationHorizontal, &glyph, NULL, 1);
    ...
    if (measureOption == ASTextNodeRendererMeasureOptionCapHeight || measureOption == ASTextNodeRendererMeasureOptionBlock) {
...
- (void)enumerateTextIndexesAtPosition:(CGPoint)position usingBlock:(as_renderer_index_block_t)block {
    if (position.x > _constrainedSize.width || position.y > _constrainedSize.height || block == NULL) {
    ...
    for (NSInteger i = startIndex; i <= endIndex && !stop; i++) {
        for (NSInteger j = startIndex; j <= endIndex && !stop; j++) {
            CGPoint currentPoint = CGPointMake(position.x + i * pointSeparation, position.y + j * pointSeparation);

            // We ask the layout manager for the proper glyph at the touch point
            NSUInteger glyphIndex = [_layoutManager glyphIndexForPoint:currentPoint inTextContainer:_textContainer];
            ...
            CGRect glyphRect = [self _rectForGlyphAtIndex:glyphIndex measureOption:ASTextNodeRendererMeasureOptionLineHeight];

            // Sometimes TextKit plays jokes on us and returns glyphs that really
            // aren't close to the point in question.  Silly TextKit...
            if (!CGRectContainsPoint(CGRectInset(glyphRect, -ASTextNodeRendererGlyphTouchHitSlop, -ASTextNodeRendererGlyphTouchHitSlop),
                                     currentPoint)) {
                continue;
            }
...
- (NSUInteger)_calculateCharacterIndexBeforeTruncationMessage {
    ...
    CGRect truncationUsedRect = [truncationComponents.layoutManager boundingRectForGlyphRange:truncationGlyphRange
                                                                              inTextContainer:truncationComponents.textContainer];
    ...
    ASDisplayNodeAssert(firstCharacterIndexToReplace != NSNotFound,
